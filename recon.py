...
class PhaseSpaceReconstructor:
    def __init__(self):
        ...
        self.image_scale_x = 0.00004 #mm/pix
    ...
    def _update_separators(self):
        ...
        self.seperators_x = seps
        seps = (self.peaks_y[:-1] + self.peaks_y[1:])/2
        seps = np.insert(seps, 0, 2*seps[0]-seps[1])
        seps = np.append(seps, 2*seps[-1]-seps[-2])
        seps = np.clip(seps, 0, self.image_filtered_marginal_y.size-1)
        seps = np.floor(seps).astype(int)
        self.seperators_y = seps

    def _update_masked_image(self):
        mask = np.zeros_like(self.image_filtered)
        mask[self.seperators_y[0]:self.seperators_y[-1], self.seperators_x[0]:self.seperators_y[-1]] = 1
        self.image_masked = self.image_filtered * mask
    ...
    def _update_maps(self):
        ...
        self.map_xp = (np.arange(self.image_filtered_marginal_x.size)*self.image_scale_x - self.map_x) / self.mask_screen_distance
        ...
        self.map_yp = (np.arange(self.image_filtered_marginal_y.size)*self.image_scale_y - self.map_y) / self.mask_screen_distance


    def _update_results(self):
        results = {}

        v2 = self.mask_hole_radius ** 2/4
        L = self.mask_screen_distance
        correct = np.array([[v2,v2/L],[v2/L,v2/L**2]])
        ...
        results["emittance_x_corrected"] = emittance_y_corrected
    ...
    def plot_reconst(self):
        plt.figure(figsize=(15,10))
        low, up = self.seperators_x[0], self.seperators_x[-1]
        plt.scatter(self.map_x[low:up], self.map_xp[low:up], c=self.image_filtered_marginal_x[low:up])
        plt.show